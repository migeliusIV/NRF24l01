...
int potValue[6]; // Создаём массив для приёма значений потенциометра
int pos = 0;    //
...
void setup() {
  ...
  radio.openReadingPipe (1, 0x4848484848LL); // Открываем трубу ID передатчика
...
void loop() {
  if(radio.available()){    // Если в буфер приёмника поступили данные
   ...
   if (katap == true && pos <= 95){
     myservo.write(pos); 
     pos += 1;
     delay(15);
   } else if (katap == false && pos > 0){
     myservo.write(pos); 
     pos -= 1;
     delay(15);
   } 
