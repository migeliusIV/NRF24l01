...
int potValue[6]; // Создаём массив для передачи значений потенциометра
...
void setup() {
  ...
  radio.setChannel(25); // Обмен данными будет вестись на пятом канале (2,405 ГГц)
  radio.setDataRate (RF24_1MBPS); // Скорость обмена данными 1 Мбит/сек
  radio.setPALevel(RF24_PA_HIGH); // Выбираем высокую мощность передатчика (-6dBm)
  radio.openWritingPipe(0x4848484848LL); // Открываем трубу с уникальным ID
}
...
void loop() {
  ...
  potValue[4] = digitalRead(3);
  potValue[5] = digitalRead(5);
