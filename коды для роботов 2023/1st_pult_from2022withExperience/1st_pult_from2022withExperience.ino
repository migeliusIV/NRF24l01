...
#define pin_btn_SM 8   // slow motion, на версиях младше 2022 поменяйте пины кнопок, которые будут снаружи пульта
#define pin_btn_FM 11   // forsage, кнопку не проверял
...
void setup() {
  ...
  pinMode(pin_btn_SM,INPUT);
  pinMode(pin_btn_FM,INPUT);
  //настройка монитора порта, чтобы отслеживать, на каком этапе ошибка
  Serial.begin(9600);
  
  //настройка радио
  radio.begin();  // Инициализация модуля NRF24L01
  radio.setChannel(10); // Обмен данными будет вестись на 16 канале (2,416 ГГц). Это объясняется тем, что каналы кодируются не в десятеричной системе счисления
