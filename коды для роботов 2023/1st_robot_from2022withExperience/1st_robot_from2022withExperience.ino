...
void setup() {
  ...
  radio.setChannel(10); // Обмен данными будет вестись на 16 канале (2,416 ГГц)
...
void command_slowmode() {
  ...
  if (motor_direction) {
    if (potValue[0] > 125 && potValue[1] > 125) {
      analogWrite(PIN_ENA, map(potValue[1], 126, 255, 0, 51));
      analogWrite(PIN_ENB, map(potValue[0], 126, 255, 0, 51));
      ...
      analogWrite(PIN_ENA, map(potValue[1], 0, 125, 51, 0));
      analogWrite(PIN_ENB, map(potValue[0], 126, 255, 0, 51));
      ...
      analogWrite(PIN_ENA, map(potValue[1], 126, 255, 0, 51));
      analogWrite(PIN_ENB, map(potValue[0], 0, 125, 51, 0));
    ...
    if (potValue[0] > 125 && potValue[1] > 125) {
      analogWrite(PIN_ENA, map(potValue[0], 126, 255, 0, 51));
      analogWrite(PIN_ENB, map(potValue[1], 126, 255, 0, 51));
      ...
      analogWrite(PIN_ENA, map(potValue[0], 0, 125, 51, 0));
      analogWrite(PIN_ENB, map(potValue[1], 126, 255, 0, 51));
      ...
      analogWrite(PIN_ENA, map(potValue[0], 126, 255, 0, 51));
      analogWrite(PIN_ENB, map(potValue[1], 0, 125, 51, 0));
...
// Парни, вы уж извините за такой спагетти-код, но над ним работало два человека в разные годы и большая часть нашей программной работы в нём и других прилагающихся кодах, 
// так что повнимательнее к деталям.
