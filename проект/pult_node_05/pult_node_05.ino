// передача не перегружает приёмник и частота в не зоны блютуза
...
#define pin_pot_1 A0  // Джойстик 1
...
void setup() {
  ...
  SPI.begin();
  radio.begin();
  network.begin(0x6a, this_node); // Обмен данными будет вестись на 16 канале (2,416 ГГц). Это объясняется тем, что каналы кодируются не в десятеричной системе счисления
...
void loop() {
  //считывание 
  potValue[0] = 255 - analogRead(pin_pot_1)/4; // Считываем показания потенциометра. Делим на 4 здесь, а не в 'map' из программы пульта, только чтобы не забыть
